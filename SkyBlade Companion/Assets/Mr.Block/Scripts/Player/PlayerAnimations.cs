...
public class PlayerAnimations : MonoBehaviour
{
    ...
    {
        BPlayerController.OnJump += PlayPoofDustVFX;
    }
    ...
    {
        if (BPlayerController.Instance.CheckGrounded())
        {
    ...
    private void ApplyTilt()
    {
        ...
        if (BPlayerController.Instance.MoveInput.x < 0f)
        {
            targetAngle = _tiltAngle;
        }
        else if (BPlayerController.Instance.MoveInput.x > 0f)
        {
            targetAngle = -_tiltAngle;
        }
        else
